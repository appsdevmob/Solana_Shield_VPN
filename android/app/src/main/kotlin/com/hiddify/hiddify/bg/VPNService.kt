...
class VPNService : VpnService(), PlatformInterfaceWrapper {
    ...
    override fun openTun(options: TunOptions): Int {
        // VPN permission is verified by MainActivity.requestVpnPermission() BEFORE the service starts.
        // VpnService.prepare(serviceContext) from a Service context cannot show a permission dialog,
        // so a retry loop here is futile. We do a single check for safety.
        val permissionIntent = prepare(this)
        if (permissionIntent != null) {
            Log.w("VPN", "android: VPN permission not granted (prepare returned non-null from service context)")
            // Send alert to Activity to request permission
            try {
                MainActivity.instance.onServiceAlert(
                    com.hiddify.hiddify.constant.Alert.RequestVPNPermission,
                    "VPN permission revoked, please reconnect"
                )
            } catch (e: Exception) {
                Log.e("VPN", "Failed to send alert to MainActivity", e)
            }
            error("android: missing vpn permission")
        }
        Log.d("VPN", "VPN permission verified, opening tun")
