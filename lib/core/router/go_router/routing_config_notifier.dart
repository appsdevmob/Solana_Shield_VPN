...
import 'package:hiddify/features/solana/widget/solana_auth_page.dart';
...
final loadingConfig = RoutingConfig(
  routes: <RouteBase>[
    ...
    GoRoute(path: '/solana-auth', builder: (context, state) => const Material()),
  ],
);
  ...
  RoutingConfig build() {
    ...
    return RoutingConfig(
      redirect: (context, state) {
        final isSolanaAuth = state.matchedLocation == '/solana-auth';
        final solanaAuthenticated = ref.read(solanaIsAuthenticatedProvider);
        ...
        // Gate: require Solana auth
        if (!solanaAuthenticated) {
          // Already on auth page → let it render
          if (isSolanaAuth) return null;
      ...
      routes: <RouteBase>[
        ...
        // Intro route removed
        GoRoute(name: 'solana-auth', path: '/solana-auth', builder: (_, _) => const SolanaAuthPage()),
      ],
    );
  }
}
